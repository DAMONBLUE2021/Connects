...
export default function Masthead() {
    ...
    return (
        <header className="w-full border-b-4 border-black mb-8 relative z-40 bg-[var(--paper)] transition-colors duration-700">
            {/* Ticker */}
            <div className="bg-[var(--ink)] text-[var(--paper)] py-1 overflow-hidden whitespace-nowrap relative z-10 transition-colors duration-700">
            ...
            <div className="container mx-auto px-4 py-8 flex flex-col items-center relative">
                ...
                {/* Navigation */}
                <nav className="hidden md:flex flex-wrap gap-8 py-4 font-sans text-xs font-bold tracking-widest uppercase border-b border-gray-200 w-full justify-center">
                    {navItems.map((item) => (
                        <Link
                            key={item.href}
                            href={item.href}
